...
// concepts of IT 
    if(isset($_REQUEST['edit_access'])){
        ...
        if(empty($question) || empty($option1) || empty($option2) || empty($option3)|| empty($option4) || empty($answer)){
            header("Location:edit_access.php?edit_id={$id}&&false");
        }else{
            $res = "UPDATE ms_access_16 SET question = '$question', option1='$option1', option2='$option2', option3='$option3', option4='$option4', answer='$answer' WHERE id = '$id'";
