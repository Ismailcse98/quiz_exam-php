...
<div class="content-section">
    <div class="container">
        ...
        <?php
            ...
            $chapter = "SELECT * FROM chapter_list WHERE chapter_name = 'Final Exam'";
...
<?php
    ...
    error_reporting(0);
        ...
        $opt16 = $_REQUEST['option16'];
        $opt17 = $_REQUEST['option17'];
        $opt18 = $_REQUEST['option18'];
        $opt19 = $_REQUEST['option19'];
        $opt20 = $_REQUEST['option20'];
        $opt21 = $_REQUEST['option21'];
        $opt22 = $_REQUEST['option22'];
        $opt23 = $_REQUEST['option23'];
        $opt24 = $_REQUEST['option24'];
        $opt25 = $_REQUEST['option25'];

        $query = "SELECT * FROM final_exam";
        ...
        if($runQuery == true){
            while($data = mysqli_fetch_array($runQuery)){
                ...
                if($opt21 == $data['answer']){
                    $i++;
                }
                if($opt22 == $data['answer']){
                    $i++;
                }
                if($opt23 == $data['answer']){
                    $i++;
                }
                if($opt24 == $data['answer']){
                    $i++;
                }
                if($opt25 == $data['answer']){
                    $i++;
                }
            }
        ...
        $wrong_ans = 25 - $i;
        $Total_mark = $i * 2;
        $insertQuery = "INSERT INTO result (chapter,name,student_id,correct_ans, wrong_ans, total_mark ) VALUES ('Final Exam','$name','$stu_id','$i','$wrong_ans','$Total_mark')";
